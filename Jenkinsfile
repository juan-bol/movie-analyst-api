            ...
            steps {
                withAWS(credentials:'aws-key') {
                    sshagent(['jenkins-key']) { // ansible private ip
                        ...
                        sh "ssh -o StrictHostKeyChecking=no ec2-user@10.1.5.209 'ansible-playbook ~/movie-ramp-up/ansible/add-key.yml -i ~/movie-ramp-up/ansible/ec2.py -l tag_Name_Back_EC2_Terra_juan_bolanosr --key-file ~/key-pair-JB.pem --user ubuntu'"
